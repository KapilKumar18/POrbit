        ...
        <div className="user_popup py-5 ">
          <div>
            ...
            <ul className="list-group list-unstyled list-group-flush user_list mx-5 my-3 ">
              {user.map((item) => (
                <Link
                  to={{ pathname: "/profile", userId: item.id }}
                  key={item.id}
            ...
            <div className="btn">
              <p
                className="text-light bg-danger text-center w-25 p-2 rounded m-auto"
                onClick={gotohome}
              >
                {" "}
                Sign out
